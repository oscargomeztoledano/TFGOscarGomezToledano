...
import { updateNivelesByCorreo } from "../../utils/apiCalls"
export function successWindow(scene, timeTaken) {
    ...
    const nextLevelIndex = scene.currentLevelIndex + 1

    if ( mundos && mundos[scene.currentWorldIndex].niveles[nextLevelIndex]) 
        mundos[scene.currentWorldIndex].niveles[nextLevelIndex].desbloqueado = true

    if ( mundos[scene.currentWorldIndex].niveles[scene.currentLevelIndex].puntos < scene.puntosBase - timeTaken) {
        mundos[scene.currentWorldIndex].niveles[scene.currentLevelIndex].puntos = scene.puntosBase - timeTaken
        mundos[scene.currentWorldIndex].niveles[scene.currentLevelIndex].estrellas = scene.stars.length
        
        updateNivelesByCorreo(usuario.correo, mundos).then((res) => {
            localStorage.setItem('usuario', JSON.stringify({...usuario, mundos: res}))
            console.log('niveles actualizados', res)
            }).catch((err) => {
    ...
    botones.forEach((boton, i) => {
        if (boton.icon === 'iconResume' && 
            !mundos[scene.currentWorldIndex].niveles[nextLevelIndex]) return
