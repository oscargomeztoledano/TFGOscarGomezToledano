...
import { getMundosByAula, guardarMundoAula } from "../../../utils/apiCalls"
export  function habilitaciones(scene,usuario) {
    ...
    const tabla = scene.add.dom(0, -30).createFromHTML(`<div id="tabla" style="width:300px;height:375px;overflow-y:auto;color:#fff">
    </div>`).setOrigin(0.5)
    container.add(tabla)
    ...
    const listaMundos = usuario.mundos.map(m => m.nombre.toUpperCase())
