...
export class mundo1nivel1_2 extends Scene
{
    ...
    update()
    {
        ...
        if (remainingCollectibles === 0 && this.controlEnabled) {
            this.controlEnabled = false
            const timeTaken = Math.floor((Date.now() - startTimeM1L1_2) / 1000)
...
function initQuestions(scene) {
    ...
    const kahootData = scene.cache.json.get('kahootLevel1_2')
    const entry = kahootData.anslevel1_2.find(q => q.id === id)
